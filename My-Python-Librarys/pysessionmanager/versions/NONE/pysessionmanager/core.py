...
class SessionManager:
    def __init__(self, protect_all: bool = False, logging: bool = False, standart_id:str=None):
        self.sessions: Dict[str, Dict] = {}
        self.filename = "sessions.json"
        self.db_name = "sessions.db"
        self.protect_all = protect_all
        self.logging = logging
        self.standart_id = standart_id
        self._Session_IDs = []
    ...
    def create_session(
        ...
        try:
            ...
            while session_id in self._Session_IDs:
                session_id = generate_session_id()
            self._Session_IDs.append(session_id)
        except Exception as e:
            print(f"Fout bij genereren van sessie-ID: {e}")
    ...
    def change_session_value(self, value: str, session_id:Optional[str]=None, ):
        """
        Change the value of a session.
        """
        if session_id == None:
            session_id = self.standart_id
        ...
        if len(value) < 1:
            raise ValueError("Value must be at least 1 character long.")
        self.sessions[session_id]["value"] = value
    ...
    def get_session(self, session_id: str=None) -> Optional[Dict]:
        """
    ...
    def get_time_remaining(self, session_id: str=None) -> float:
        """
    ...
    def time_passed(self, session_id: str=None) -> float:
        """
    ...
    def get_all_sessions(self) -> Dict[str, Dict]:
        ...
        }, removed_sessions, protected_sessions if not protected_sessions else None
